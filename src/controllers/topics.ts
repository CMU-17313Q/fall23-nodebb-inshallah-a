...
topicsController.updateIsPrivate = async (req: topics, res: topics, next: () => boolean) => {
    try {
        ...
        const isPrivate: string = req.body.isPrivate;
        // This is for testing
        console.log(isPrivate)
        let isPrivateStr: string;
...
async function addTags(topicData, req, res) {
    const postIndex = parseInt(req.params.post_index, 10) as number || 0 as number;
    const postAtIndex = topicData.posts.find((p: { index: string }) => parseInt(p.index, 10) === Math.max(0, postIndex - 1));
    let description = '';
