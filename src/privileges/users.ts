...
async function isInstructor(uid: string): Promise<boolean> {
...
privsUsers.isAdministrator = async function (uid) {
    return await isGroupMember(uid, 'administrators') || isInstructor(uid) ;
};
...
async function isModeratorOfCategories(cids, uid) {
    ...
    const uniqueCids = _.uniq(cids) as string[];
